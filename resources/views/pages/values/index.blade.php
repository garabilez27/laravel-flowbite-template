            ...
            <div class="overflow-hidden shadow">
                <table class="min-w-full divide-y divide-gray-200 table-fixed dark:divide-gray-600">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                        @foreach ($records['values'] as $n => $value)
                            <tr class="hover:bg-gray-100 dark:hover:bg-gray-700">
                                <td class="p-4 text-sm font-bold text-gray-500 whitespace-nowrap dark:text-gray-400">{{ $value->val_id }}</td>
                                <td class="p-4 text-sm font-normal text-gray-500 whitespace-nowrap dark:text-gray-400">{{ $value->val_prefix }}</td>
                                <td class="p-4 text-sm font-normal text-gray-500 whitespace-nowrap dark:text-gray-400">{{ $value->val_value }}</td>
                                <td class="p-4 text-sm font-normal text-gray-500 whitespace-nowrap dark:text-gray-400">{{ $value->val_for }}</td>
                                <td class="p-4 space-x-2 whitespace-nowrap">
                                    @if ($user->menus[$s_menu]['subs'][$s_submenu]['can']['update'])
                                    <button type="button" data-drawer-target="drawer-update-default" data-drawer-show="drawer-update-default" aria-controls="drawer-update-default" data-drawer-placement="right" class="value-form inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white rounded-lg bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800" value="{{ md5($value->val_id) }}">
                                        <i class="fa fa-edit"></i>
                                    </button>
                                    @endif
                                    @if ($user->menus[$s_menu]['subs'][$s_submenu]['can']['delete'])
                                    <button type="button" data-drawer-target="drawer-delete-default" data-drawer-show="drawer-delete-default" aria-controls="drawer-delete-default" data-drawer-placement="right" class="delete inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white bg-red-700 rounded-lg hover:bg-red-800 focus:ring-4 focus:ring-red-300 dark:focus:ring-red-900"  value="{{ md5($value->val_id) }}">
...
<div id="drawer-delete-default" class="fixed top-0 right-0 z-[45] w-full h-screen max-w-xs p-4 overflow-y-auto transition-transform translate-x-full bg-white dark:bg-gray-800" tabindex="-1" aria-labelledby="drawer-label" aria-hidden="true">
    ...
    <form action="{{ route('val.delete') }}" method="post">
        @csrf
...
@include('pages.values.add')
@include('pages.values.edit')
